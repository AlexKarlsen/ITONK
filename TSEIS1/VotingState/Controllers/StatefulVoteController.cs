...
namespace VotingState.Controllers
{
    ...
    {
        const string activityHeader = "activity-id"; // Keep an instance of the service.
        ...
        public async Task<HttpResponseMessage> GetAsync()
        {
            ...
            ServiceEventSource.Current.ServiceRequestStart("VotesController.Get", activityId); IReadOnlyList<VotingData> counts = await _service.GetVotingDataAsync(activityId,
            CancellationToken.None);
            List<KeyValuePair<string, int>> votes = new List<KeyValuePair<string, int>>(counts.Count); foreach (VotingData data in counts)
            {
        ...
        {
            string activityId = GetHeaderValueOrDefault(Request, activityHeader, () => { return Guid.NewGuid().ToString(); });
            ServiceEventSource.Current.ServiceRequestStart("VotesController.Post", activityId); // Update or add the item.
            await _service.AddVoteAsync(key, 1, activityId, CancellationToken.None);
            ServiceEventSource.Current.ServiceRequestStop("VotesController.Post", activityId); _service.RequestCount = 1;
        ...
        public static string GetHeaderValueOrDefault(HttpRequestMessage request, string headerName, Func<string> getDefault)
        {
            ...
            if ((null == request) || (null == request.Headers)) return getDefault();
