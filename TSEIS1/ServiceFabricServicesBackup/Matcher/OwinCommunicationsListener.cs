...
namespace Matcher
{
    ...
    {
        private readonly ServiceEventSource eventSource; private readonly StatefulServiceContext serviceContext; private readonly string endpointName;
        private readonly string appRoot;
        private IDisposable webApp;
        private string publishAddress;
        private string listeningAddress;
        private readonly Common.IStockBidService serviceInstance;
        ...
        {
            if (serviceContext == null) throw new ArgumentNullException(nameof(serviceContext)); if (endpointName == null) throw new ArgumentNullException(nameof(endpointName));
        ...
        public Task<string> OpenAsync(CancellationToken cancellationToken)
        {
            ...
            if (this.serviceContext is StatefulServiceContext)
            {
                ...
                this.listeningAddress = string.Format(CultureInfo.InvariantCulture, "http://+:{0}/{1}{2}/{3}/{4}", port,
                    string.IsNullOrWhiteSpace(this.appRoot) ? string.Empty
                    : this.appRoot.TrimEnd('/') + '/', statefulServiceContext.PartitionId, statefulServiceContext.ReplicaId, Guid.NewGuid());
            }
            else
            {
                throw new InvalidOperationException();
            }
            publishAddress = this.listeningAddress.Replace("+",
            FabricRuntime.GetNodeContext().IPAddressOrFQDN);
            try
            {
                this.eventSource.ServiceMessage(this.serviceContext, "Starting web server on " + this.listeningAddress);
                this.webApp = WebApp.Start(this.listeningAddress, ConfigureApp); this.eventSource.ServiceMessage(this.serviceContext, "Listening on " + this.publishAddress);
        ...
        {
            this.eventSource.ServiceMessage(this.serviceContext, "Closing web server"); this.StopWebServer();
        ...
        // This private code configures Web API.This is called from Web.Start.
        void ConfigureApp(IAppBuilder appBuilder) // Configure Web API for self-host.
        {
        ...
        /// <param name="controllerType">The type of the controller that was selected for this request</param> /// <returns>An instance of the selected HTTP controller</returns>
